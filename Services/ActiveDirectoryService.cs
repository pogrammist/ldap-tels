...
public class ActiveDirectoryService
{
    ...
    public ActiveDirectoryService(IConfiguration configuration, ILogger<ActiveDirectoryService> logger)
    {
        ...
        _adminGroup = _configuration["ActiveDirectory:AdminGroup"] ?? throw new ArgumentNullException("ActiveDirectory:AdminGroup");
    }
    ...
    public bool IsUserInAdminGroup(string username)
    {
        try
        {
            ...
            using (context)
            {
                ...
                _logger.LogInformation("Поиск группы {GroupName}", _adminGroup);
                using var group = GroupPrincipal.FindByIdentity(context, IdentityType.Name, _adminGroup);
                if (group == null)
                {
                    _logger.LogWarning("Группа {GroupName} не найдена", _adminGroup);
                    return false;
                }
    ...
    public string GetUserEmail(string username)
    {
        ...
        {
            _logger.LogError(ex, "Ошибка при получении email для пользователя {Username}", username);
            return string.Empty;
        }
    }
    ...
    //     catch (Exception ex)
    //     {
    //         _logger.LogError(ex, "Ошибка при получении отдела для пользователя {Username}", username);
    //         return string.Empty;
    //     }
    // }
}
