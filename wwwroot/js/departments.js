async function changeDepartmentWeight(departmentId, delta) {
    try {
        const urlInput = document.getElementById('update-department-weight-url');
        if (!urlInput) {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({ id: departmentId, delta: delta })
        });
        ...
        const weightElement = document.getElementById(`department-weight-${departmentId}`);
